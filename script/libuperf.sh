...
uperf_start() {
    ...
    [ -f "$USER_PATH/uperf_log.txt" ] && mv "$USER_PATH/uperf_log.txt" "$USER_PATH/uperf_log.txt.bak"
    if [ -f "$BIN_PATH/libc++_shared.so" ]; then
        ASAN_LIB="$(ls $BIN_PATH/libclang_rt.asan-*-android.so 2>/dev/null | head -n 1)"
        [ -n "$ASAN_LIB" ] && export LD_PRELOAD="$ASAN_LIB $BIN_PATH/libc++_shared.so"
    fi
