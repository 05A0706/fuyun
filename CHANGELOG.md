...
> ⚠️ **26w34.6-B 起「频率压制」已删除**：`freq_limit.sh`、`freq_limit.txt`、`freq_range.txt`、`screen_saver.txt`
> 及其 WebUI 卡片均已移除。下方 26w34.6-b 及更早条目中提到的「频率限制」「CPU 频率范围」「息屏压频」
> 均为**历史记录**，实际功能已由「核心开关」取代。

## fuyun 26w34.6-B 第五轮修订：配置简易化 (20260829, 未发布)

### 配置文件合并 (11 个 → 7 个)
- **`fuyun.conf`**：原 `mem_config.txt` + `idle_gov.txt` + `corectl.txt` 合并，分区 `[mem]` / `[idle_gov]` / `[corectl]`
- **`whitelist.txt`**：原 `mem_whitelist.txt` + `idle_whitelist.txt` + `doze_whitelist.txt` 合并，分区 `[mem]` / `[idle_gov]` / `[doze]`
- 保留独立：`uperf.json` / `cur_powermode.txt` / `perapp_powermode.txt` / `mem_apps.txt` / `automation.txt`
- **自动迁移**：升级时旧文件自动合并到新文件（幂等，原文件保留 `.legacy` 后缀可回退）；`setup.sh` 安装改为复制新模板
- 全部守护脚本（memctl/auxgov/corectl/automation/service/post-fs-data）与 WebUI（lib/status/6 个写配置 CGI/config_file/backup）改读分区

### Ordinary / Expert 分级
- WebUI 默认只显示高频项：模式、预设、核心开关总开关+关核数、内存总开关+模式+空闲淘汰、调速器总开关
- 低频项折叠进「专家」区（默认收起）：PSI/轮询间隔/每轮进程数/推送清理、调速器 4 参数、息屏/联动关核
- 新增「配置预设」卡片：均衡默认 / 日常省电 / 游戏性能 / 极速 一键应用，支持把当前配置保存为自定义预设（`/data/adb/uperf/presets/`），应用即时生效核心开关

### 基础设施
- `libcommon.sh` 新增分区工具：`section_body` / `set_section_kv` / `get_section_kv` / `migrate_legacy`
- `lib.sh` 新增白名单分区行操作：`add_section_line` / `del_section_line`（去注释匹配去重/删除）
- 新增 `cgi-bin/preset.sh`（预设 list/apply/save/delete，内置四档 + 自定义）
- 高级设置编辑器改为 `fuyun.conf` / `whitelist.txt` / perapp / uperf / automation 五项
- README 配置表与快速上手重写（WebUI 优先）

## fuyun 26w34.6-B 第四轮修订 (20260829, 未发布)

### 新增机型特调 (资料文件夹接入)
- 接入 5 份 uperf 特调配置：新增 **骁龙 8 Gen3 (sdm8g3)**、**8 Elite (sdm8e)**、**8s Elite (sdm8e5)**，`sdm8+` / `sdm8g2` 更新为资料版（新增 games_moba 等规则、dynamic_boost、正则锚定）
- `libsysinfo.sh` 补齐 board 映射：`sun`→sdm8e、`bonito/sun2/voltron`→sdm8e5（真机 board 名如有出入可在 get_config_name 自行补充）

### 插件系统升级 (WebUI 插件管理器 + 配置插件 + 示例插件)
- 新增 `webroot/cgi-bin/plugins.sh`：插件列表 / 启用 / 停用 / 删除 / 查看 plugin.log
- **配置插件 (特调化)**：插件目录放 `uperf.<名称>.json` 即可在 WebUI 一键应用（自动备份当前配置 + 重启 uperf + 清除辅助调速器深度状态）
- 内置示例插件 `plugin_examples/example_plugin.sh`（默认停用，演示阶段与环境变量）
- `plugin.sh`：`.disabled` 后缀 = 停用（自动跳过）；`uperf.*.json` 配置插件不自动执行；插件日志轮转；README.txt 更新三种插件说明
- 管理页新增「插件管理」卡片

### 修复 (第四轮评审 25 项)
- **卸载残留**：uninstall.sh 增加停止 automation.sh / service.sh 看门狗；perapp 保留判存在+引号；模块路径改 $MODDIR 推导
- **路径硬编码**：lib.sh / action.sh 模块路径改为从 webroot 向上推导
- **CGI 校验与守护对齐**：set_mem_cfg.sh（原 set_config.sh）PUSH_KEEP/KEEP_CMDLINE 字符集、INTERVAL/PSI/MAX_PER_ROUND/IDLE_KILL_MIN 范围与 memctl.sh 一致；set_idle_cfg.sh IDLE_INTERVAL≥3 / IDLE_CPU_THD≤100
- **auxgov 深度参数残留**：进入深度空闲记录 idle 功率原值，退出时逐键还原（用户改过 uperf.json 也正确合并，不再残留深度参数）
- **backup.sh 导入防御**：拒绝 tar 包内符号链接/设备/绝对路径条目，解压后双重校验
- **memctl**：clean_push_apps 的 kill -9 改 am kill（保留任务栈）；IDLE_KILL_MIN=0 时跳过闲置表计算；"立即回收一轮"改为短间隔探测即时响应（不再等满 300s）
- **健壮性**：post-fs-data.sh getprop SDK 空值加固；powercfg_once.sh rmdir 静默；webuid.sh 端口探测不再依赖 nc -z；status.sh 前台缓存文件 chmod 600
- **代码质量**：get_fg_pkg / wl_rules / wl_hit / rotate_log 抽入 libcommon.sh（memctl/auxgov/corectl/automation 复用）；删除 libcommon.sh 死代码 read_cfg_value/write_panel/clear_panel；命名统一 set_config.sh→set_mem_cfg.sh
- **WebUI 功能**：新增「重启 uperf」按钮与动作（保存 uperf.json 生效入口）；Vulkan 状态展示 + 管理页切换卡片；分应用性能模式可视化编辑（perapp.sh）；场景自动化快捷规则卡片（一键添加常用规则/清空）；日志行数选择 + 自动刷新；footer 显示模块版本；uperf.json 前端括号校验；后端提示文案统一中文
- **文案/文档**：README 插件路径修正（/data/adb/uperf/plugins）；核心开关安全说明统一为"绝不关 cpu0、小核始终在线（大/中核可整簇关闭）"（README/index.html/setup.sh/CHANGELOG）

## fuyun 26w34.6-B 功能增强 (20260829, 未发布)

### F7 统一配置校验 + 原子写入 + 回滚
- `cgi-bin/lib.sh` 新增 `valid_bool/valid_uint/valid_int/valid_pkg/valid_pkg_prefix/valid_enum/valid_range`、`sed_escape`、`atomic_write/atomic_sed/set_kv_file`
- 所有写配置 CGI 复用统一校验与原子写入（写前自动 `.bak`），消除"写坏 `uperf.json`→调度失效"与包名正则未转义等边界缺失
- `config_file.sh` 增加 POST 体积上限 (1MB) 与 uperf.json 括号平衡校验

### F5 WebUI API 访问认证（安全加固）
- `webuid.sh` 启动时生成 root-only 令牌 (`/data/adb/uperf/.webui_token`, 0600) 并注入 `index.html`
- `lib.sh` 新增 `require_token`，全部 CGI 入口统一校验；页面 `api()/apiPost()` 自动携带令牌
- 说明：回环接口被任意本地 App 访问属已知架构限制，此为纵深防御；完整修复需管理器外带令牌

### F6 插件隔离与准入控制（安全加固）
- 插件目录由共享存储 `/sdcard/.../plugins` 迁至 root-only `/data/adb/uperf/plugins`（仅 root 可写）
- 取消 JSON `command` 字段直通 `sh -c`；仅允许 `file` 指向插件目录内脚本，并校验路径无 `..` 逃逸、非他人可写
- `install.sh` 增加迁移提示

### F8 进程看门狗 + 统一日志与轮转
- `service.sh` 新增看门狗：监督 memctl / auxgov / corectl(watch) 异常退出自动拉起
- `corectl.sh` / `auxgov.sh` 日志从 `mem_log.txt` 拆出独立文件 (`corectl.log.txt` / `auxgov.log.txt`) 并统一轮转
- `libcommon.sh` 新增 `rotate_log`，`service.sh` 日志复用；日志下拉新增 corectl/auxgov/watchdog/auto 选项

### F3 配置导入/导出与一键备份
- 新增 `cgi-bin/backup.sh`：打包全部配置为带版本号 `tar.gz`（base64 返回），导入前自动 `.bak`、仅允许白名单文件防穿越
- 管理页新增「导出配置 / 导入配置」按钮

### F4 实时运行曲线
- 新增 `cgi-bin/metrics.sh`（自包含频率分类，输出大/中/小核平均频率 + 可用内存 + 温度）
- 新增 `metrics.js`：原生 Canvas 绘制实时曲线（每 2 秒采样，仅状态页可见时轮询，零图表库）

### F2 场景自动化规则引擎
- 新增 `script/automation.sh` 守护：按 充电/电量/时段/屏幕 触发策略，与 perapp(按前台应用) 维度互补
- 规则文件 `automation.txt`；管理页「高级」新增 `automation.txt` 编辑器（复用 `config_file.sh` 原子保存）

## fuyun 26w34.6-B (20260829)

### 删除频率压制
- **移除** `freq_limit.sh` 的 bind-mount 冻结机制：`FREQ_CAP` / `FREQ_RANGE` / `FREQ_OFFSCREEN` 及其掩码文件、
  频点校验、分簇识别等压频专用逻辑全部删除（脚本文件本体一并移除）
- 删除配置文件 `config/freq_limit.txt`、`config/freq_range.txt`
- 删除 `setup.sh` 的 `choose_screen_saver()` 及 `screen_saver.txt` 遗留接口（早已无对应功能）
- 卸载脚本改为停止 `corectl.sh` 并恢复全部核心在线

### 新增核心开关（热插拔关核）
- 新脚本 `script/corectl.sh`，配置 `/sdcard/Android/yc/uperf/corectl.txt`
  - `CORECTL_ENABLE` 总开关（默认 0，由用户自行开启）
  - `BIG_OFF` / `MID_OFF` 常态关大核 / 中核数量
  - `OFFSCREEN_OFF` 息屏额外关核（用户选择，默认 0）
  - `IDLE_OFF` 辅助调速器深度空闲联动关核（默认 1）
  - `KEEP_LITTLE` 小核始终在线
- 安全约束：**绝不关 cpu0**、**小核始终在线**（大/中核可整簇关闭，系统始终保有一整簇算力）、`clear` 一键全恢复
- CLI：`watch` / `apply` / `clear` / `--off <idle|screen>` / `--on <idle|screen>` / `status`
- 省电主杠杆由"压频率"改为"关核心"

### 辅助调速器独立成模块
- `memctl.sh` 中的 idle 调速逻辑抽出为独立守护 `script/auxgov.sh`，由 `service.sh` 启动
- 配置 / 白名单 / 状态文件路径不变：`idle_gov.txt`、`idle_whitelist.txt`、`idle_gov.state`，日志前缀仍为「辅助调速」
- 深度空闲时联动 `corectl.sh` 关核（受 `IDLE_OFF` 与 `CORECTL_ENABLE` 约束），退出深度空闲即恢复
- 修复 `get_fg_pkg_cached` 悬空调用（抽离时遗漏该函数与 `FG_CACHE` 变量，仅 `sh -n` 检查无法发现）

### 内存管理优化
- `memctl.sh` 由约 1000 行精简至 637 行，职责收敛为纯内存回收，不再承担频率相关逻辑

### WebUI
- 新增 `corectl.sh` CGI 与「核心开关」卡片，替代原「频率限制」「CPU 频率范围」两张卡片
- 关核数量滑块上限按设备实际核数动态收敛（至少保留 1 核）
- 底部页签「频率」改为「核心」
- 文案清理：「压制频率」「空闲压频」统一改为「降频 / 关核」「辅助调速」
